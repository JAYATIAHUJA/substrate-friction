...
*Generated by `scripts/render_assets.py`.*
...
The unconfirmed edges are not random. They cluster on container-method names that collide across the codebase (`extend` 139, `lower` 125, `cursor` 54, `import_module` 33, `search` 31 — full offender table and plot in `docs/graph-delta.md` / `docs/plots/offenders.png`), and the full 1,492 are classified by deterministic rules in **the wrong-edge taxonomy** (`docs/edge-taxonomy.md`, study S3): `unique_unconfirmed` **70.5%**, `builtin_method` 21.4%, `ambiguous_name` 7.9%, `dunder` 0.1%.
