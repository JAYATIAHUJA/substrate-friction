...
def substrate_tree_with_data():
    """substrate_tree plus a compact JS cell-data emit for the live
    background (docs/tree-data.js) — one generator, two consumers."""
    svg_text, cells = substrate_tree(return_cells=True)
    flat = ",".join(f"{gx},{gy},{min(7, int(h * 8))}"
                    for (gx, gy), h in sorted(cells.items()))
    js = ("// generated by scripts/render_assets.py — do not edit\n"
          "window.__TREE={cell:11,w:1600,h:760,seed:20260819,"
          f"cells:[{flat}]}};\n")
    return svg_text, js


def substrate_tree(return_cells=False):
    ...
    return (out, cells) if return_cells else out
...
def main() -> None:
    ...
    (PLOTS / "budget-curves.svg").write_text(
        ...
        encoding="utf-8")
    svg_text, cells_js = substrate_tree_with_data()
    (PLOTS / "substrate-tree.svg").write_text(svg_text, encoding="utf-8")
    pathlib_docs = PLOTS.parent
    (pathlib_docs / "tree-data.js").write_text(cells_js, encoding="utf-8")
