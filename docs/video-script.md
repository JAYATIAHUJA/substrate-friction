...
| 1:25–1:50 | [STILL] `docs/plots/direction.png` — the 0 / 55 / 98 bars. Caption: "test → fix, not fix → test — and COVERS, traced live, moves 11/18 → 12/18." | "Does the graph connect a fix to the test that exercises it? Fix to test, directed: zero of forty-four — code doesn't call tests, the direction was backwards. Test to fix: twenty-four. Undirected: forty-three — fixture machinery a static graph can't see. We also traced it live, the executed edge: test-to-fix goes eleven of eighteen to twelve — real, modest, and it doesn't rescue the predictor." | 63 |
